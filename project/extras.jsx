...
/* ---------- SectionFx: декоративный слой внутри секции ----------
   Разметки нет вовсе: сам рисунок живёт в ::before/::after, вся работа в
   section-fx.css — там же объяснено, почему этот слой не возвращает тёмные
   полосы на стыках. Здесь важно одно: слой должен быть ПЕРВЫМ ребёнком
   секции, а самой секции нужен position: relative — иначе inset: 0
   отсчитается не от неё. */
function SectionFx({ variant = 'mesh', flip = false }) {
  /* Только разметка. Всё остальное — в section-fx.css (сам рисунок) и
     section-fx.js (пауза вне экрана, отключение на слабых устройствах).
     Логика там, а не здесь, потому что на внутренних страницах React нет
     вовсе, а поведение должно быть одинаковым на всём сайте. */
  return <div className={'sec-fx fx-' + variant + (flip ? ' flip' : '')} aria-hidden="true" />;
}
