...
/* Слабое устройство — декор не рисуем вовсе.
   Замер прокрутки при вчетверо замедленном процессоре: со слоями средний кадр
   78мс, без них 60мс. Половину этой разницы даёт маска, которая гасит декор у краёв
   секции, — а снять её нельзя, без неё возвращаются полосы на стыках (проверил
   детектором: три штуки, перепад до шести единиц тона).

   Поэтому выбор простой: на машине, которая это не тянет, украшения не нужны —
   нужна плавная прокрутка. Порог по числу ядер и памяти, а не по ширине экрана:
   узкий экран бывает и у мощного телефона. Свойства поддерживаются не везде,
   при их отсутствии считаем устройство нормальным — иначе декор пропадёт у
   всех пользователей Safari разом.

   Порог намеренно низкий. Первая версия отсекала всё до четырёх ядер
   включительно — и выключила декор на обычном рабочем ноутбуке, я это поймал
   только потому, что замер вдруг показал подозрительно хорошие цифры: декора
   в нём просто не было. Два ядра или два гигабайта — это уже действительно
   слабое устройство, и там разница в плавности важнее украшений. */
function weakDevice() {
...
function SectionFx({ variant = 'mesh', flip = false }) {
  ...
  /* Решение принимается один раз при первом рендере и дальше не меняется:
     перепроверять нечего, а пересчёт на каждый экземпляр — лишняя работа. */
  const [weak] = useStateE(weakDevice);
  useEffectE(() => {
