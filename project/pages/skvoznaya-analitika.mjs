...
const WORKS = [
  ...
  ['UTM-разметка и связка с CRM',
    'Единая схема меток по всем каналам — иначе отчёты не сходятся между собой. Источник передаётся в карточку сделки, и выручка возвращается обратно в отчёт по кампаниям. Сюда же заводятся обращения из мессенджеров: заявка из <a href="/razrabotka-botov/">бота</a> должна попадать в ту же воронку, что и заявка с сайта, иначе канал выглядит бесплатным.'],
