/* AUTO-GENERATED from content.json — fallback loaded before the app. */
window.CONTENT = {
  "seo": {
    "title": "Даниил Карацапов — частный маркетолог | Яндекс Директ, VK Ads, сайты",
    "description": "Частный интернет-маркетолог Даниил Карацапов: контекстная реклама в Яндекс Директ, таргет VK Ads, сайты и сквозная аналитика. В digital с 2019 года. Ведение от 30 000 ₽/мес.",
    "ogTitle": "Даниил Карацапов — частный интернет-маркетолог",
    "ogDescription": "Контекстная реклама, таргет VK Ads, сайты и аналитика. В digital с 2019 года, 70+ ниш с измеримым результатом. Работаю лично, без агентских наценок."
  },
  "hero": {
    "eyebrow": "Даниил Карацапов · интернет-маркетолог",
    "titleLine1": "Интернет-маркетинг,",
    "titleLine2": "который делаю",
    "titleAccent": "я сам",
    "sub": "Помогаю малому и среднему бизнесу привлекать клиентов через интернет. Настраиваю Яндекс Директ, VK Ads, делаю сайты — лично, без посредников и пустых отчётов.",
    "ctaPrimary": "Обсудить задачу",
    "ctaTelegram": "Telegram",
    "telegramUrl": "https://t.me/Daniil_065",
    "trust": [
      [
        "С 2019",
        "в digital-маркетинге"
      ],
      [
        "70+",
        "ниш с результатами"
      ],
      [
        "ROI 264–520%",
        "по опубликованным кейсам"
      ]
    ]
  },
  "about": {
    "eyebrow": "Обо мне",
    "heading": "Частный интернет-маркетолог,\nа не агентство",
    "paragraphs": [
      "В digital-маркетинге с 2019 года: прошёл три агентства — от младшего специалиста до тимлида команды контекстологов. С 2025 года работаю на себя и каждый проект веду лично — от аудита до результата. Настраиваю контекстную рекламу и таргет, делаю сайты для малого бизнеса, стартапов и локальных компаний по всей России.",
      "Никаких агентских наценок, «наших специалистов» и менеджеров-посредников. Вы всегда знаете, кто настраивает вашу рекламу и почему она работает именно так. Я отвечаю за результат лично."
    ],
    "chips": [
      "Онлайн по всей России",
      "Отвечаю в течение дня",
      "Веду проект лично, от А до Я"
    ],
    "stats": [
      {
        "value": 7,
        "suffix": "+",
        "label": "лет в digital-маркетинге"
      },
      {
        "value": 22,
        "suffix": "",
        "label": "кейса с измеримым результатом"
      },
      {
        "value": 3,
        "suffix": "+ млн ₽",
        "label": "оборот кабинетов в месяц"
      },
      {
        "value": 9,
        "suffix": "",
        "label": "сертификатов Яндекса и Google"
      }
    ]
  },
  "servicesHead": {
    "eyebrow": "Что я делаю",
    "heading": "Услуги интернет-маркетинга:\nреклама, сайты, аналитика",
    "lead": "Не «полный спектр услуг», а конкретные инструменты под вашу задачу. Беру то, что принесёт заявки."
  },
  "services": [
    {
      "icon": "IconTarget",
      "name": "Контекстная реклама",
      "tag": "Yandex Ads, Google Ads",
      "result": "Настрою и веду контекстную рекламу в Яндекс Директ — целевые заявки с первых дней без слива бюджета на нецелевой трафик.",
      "works": [
        "Аудит ниши, спроса и конкурентов",
        "Сбор семантики, кластеризация, минус-слова",
        "Настройка кампаний: Поиск, РСЯ, ретаргетинг",
        "Написание и A/B-тест объявлений",
        "Еженедельная оптимизация ставок и отчёт"
      ],
      "term": "Запуск за 3–5 дней",
      "price": "от 30 000 ₽/мес — ведение",
      "url": "/kontekstnaya-reklama/"
    },
    {
      "icon": "IconUsers",
      "name": "Таргетированная реклама",
      "tag": "VK Ads, Telegram Ads, Avito Ads",
      "result": "Настрою таргетированную рекламу в VK Ads, Telegram Ads, Avito — нахожу вашу аудиторию в соцсетях и веду до заявки.",
      "works": [
        "Анализ и сегментация целевой аудитории",
        "Настройка рекламного кабинета VK Ads",
        "Креативы и тексты под каждый сегмент",
        "Тест аудиторий, плейсментов и связок",
        "Масштабирование того, что приносит лиды"
      ],
      "term": "Запуск за 4–6 дней",
      "price": "от 18 000 ₽/мес — ведение",
      "url": "/targetirovannaya-reklama/"
    },
    {
      "icon": "IconMonitor",
      "name": "Разработка сайтов",
      "tag": "Лендинги и корпоративные",
      "result": "Делаю сайты под ключ — которые продают, а не просто красивые.",
      "works": [
        "Прототип и структура под одну цель",
        "Дизайн в вашем фирменном стиле",
        "Адаптивная вёрстка под все устройства",
        "Подключение форм, аналитики и CRM",
        "Базовое SEO и скорость загрузки"
      ],
      "term": "Лендинг 7–14 дней",
      "price": "лендинг от 60 000 ₽ · корпоративный от 120 000 ₽",
      "url": "/razrabotka-sajtov/"
    },
    {
      "icon": "IconChart",
      "name": "Сквозная аналитика",
      "tag": "Настройка и контроль",
      "result": "Настраиваю так, чтобы было видно, откуда приходит каждая заявка.",
      "works": [
        "Яндекс.Метрика, цели и события",
        "Коллтрекинг и отслеживание форм",
        "Интеграция рекламы с CRM",
        "Сборка наглядного дашборда",
        "Обучу читать отчёты без аналитика"
      ],
      "term": "Настройка 5–10 дней",
      "price": "сопровождение от 12 000 ₽/мес"
    },
    {
      "icon": "IconLayers",
      "name": "Комплексный маркетинг",
      "tag": "Проект целиком",
      "result": "Беру проект целиком и веду от А до Я — одна точка ответственности.",
      "works": [
        "Стратегия и медиаплан под бизнес",
        "Все каналы привлечения под ключ",
        "Сайт + реклама + аналитика в связке",
        "Еженедельные созвоны по результатам",
        "Один ответственный за весь результат — я"
      ],
      "term": "Старт за 1–2 недели",
      "price": "от 90 000 ₽/мес · зависит от объёма работ"
    }
  ],
  "processHead": {
    "eyebrow": "Как я работаю",
    "heading": "Как я работаю:\nот аудита до заявок",
    "lead": "Вы всегда понимаете, на каком этапе проект и что я делаю прямо сейчас."
  },
  "process": [
    {
      "n": "01",
      "title": "Консультация",
      "icon": "IconPhone",
      "desc": "Разбираю вашу задачу лично. Договор обсуждаем здесь же — без лишних шагов."
    },
    {
      "n": "02",
      "title": "Аудит",
      "icon": "IconSearch",
      "desc": "Смотрю, что уже работает, а что сливает бюджет. Честно и по цифрам."
    },
    {
      "n": "03",
      "title": "Стратегия",
      "icon": "IconMap",
      "desc": "Составляю план под ваш бизнес и бюджет — без шаблонных решений."
    },
    {
      "n": "04",
      "title": "Реализация",
      "icon": "IconBolt",
      "desc": "Делаю руками сам: настраиваю рекламу, собираю сайты, веду аналитику."
    },
    {
      "n": "05",
      "title": "Отчёт и итог",
      "icon": "IconChart",
      "desc": "Показываю результат на понятных метриках. Не отчёты ради отчётов."
    }
  ],
  "quotesHead": {
    "eyebrow": "Во что я верю"
  },
  "quotes": [
    {
      "text": "Цель маркетинга — сделать продажи излишними.",
      "hl": "сделать продажи излишними",
      "author": "Питер Друкер",
      "role": "теоретик менеджмента"
    },
    {
      "text": "Ваш бренд — это то, что о вас говорят, когда вас нет в комнате.",
      "hl": "о вас говорят, когда вас нет в комнате",
      "author": "Джефф Безос",
      "role": "основатель Amazon"
    },
    {
      "text": "Если это не продаёт — это не креатив.",
      "hl": "не продаёт",
      "author": "Дэвид Огилви",
      "role": "«отец рекламы»"
    },
    {
      "text": "Будьте эталоном качества — не все привыкли к среде, где ждут совершенства.",
      "hl": "эталоном качества",
      "author": "Стив Джобс",
      "role": "сооснователь Apple"
    }
  ],
  "casesHead": {
    "eyebrow": "Кейсы · доказательства",
    "heading": "Кейсы: результаты\nрекламных кампаний в цифрах",
    "lead": "Выберите направление — кейсы отфильтруются по нему. Все цифры взяты из реальных проектов."
  },
  "caseFilters": [
    {
      "id": "all",
      "label": "Все"
    },
    {
      "id": "context",
      "label": "Контекстная реклама"
    },
    {
      "id": "site",
      "label": "Разработка сайтов"
    },
    {
      "id": "target",
      "label": "Таргетированная реклама"
    },
    {
      "id": "geo",
      "label": "GEO-сервисы"
    }
  ],
  "cases": [
    {
      "id": "everest",
      "client": "Эверест Сибирия",
      "field": "Ресторан премиум-класса",
      "geo": "Москва",
      "img": "assets/cases/everest-siberia.jpg",
      "tags": [
        "context",
        "geo",
        "horeca",
        "analytics"
      ],
      "services": [
        "Яндекс Директ",
        "Яндекс Бизнес",
        "Сквозная аналитика"
      ],
      "summary": "Продвижение ресторана премиум-класса на Садовом кольце у Патриарших прудов. Стабильный поток броней, рост среднего чека и полный контроль окупаемости рекламы.",
      "metrics": [
        [
          "+64%",
          "броней за первые 3 месяца"
        ],
        [
          "ROI 298%",
          "окупаемость рекламы"
        ],
        [
          "−27%",
          "стоимость лида (CPL)"
        ],
        [
          "+19%",
          "средний чек"
        ]
      ]
    },
    {
      "id": "nebo",
      "client": "Nebo Lounge",
      "field": "Lounge Bar",
      "geo": "Москва",
      "img": "assets/cases/nebo.jpg",
      "tags": [
        "context",
        "geo",
        "horeca",
        "analytics"
      ],
      "services": [
        "Яндекс Директ",
        "Яндекс Бизнес",
        "Сквозная аналитика"
      ],
      "summary": "Комплексная стратегия привлечения гостей для премиального Lounge Bar.",
      "metrics": [
        [
          "+90%",
          "бронирований за 4 месяца"
        ],
        [
          "ROI 520%",
          "с учётом повторных визитов"
        ],
        [
          "170+",
          "броней в месяц из Директа"
        ],
        [
          "40%",
          "новых клиентов из Яндекс Бизнес"
        ]
      ]
    },
    {
      "id": "school",
      "client": "Школа «Дмитровский»",
      "field": "Частная школа",
      "geo": "Москва",
      "img": "assets/cases/school.jpg",
      "tags": [
        "site",
        "context",
        "geo",
        "target",
        "analytics"
      ],
      "services": [
        "Сайт",
        "Яндекс Директ",
        "VK Ads",
        "Яндекс Бизнес",
        "Сквозная аналитика"
      ],
      "summary": "Полный Digital-маркетинг для частной школы. Стабильный поток заявок от родителей Москвы и выше наполняемость групп.",
      "metrics": [
        [
          "+400%",
          "посетителей сайта за 4 месяца"
        ],
        [
          "ROI 322%",
          "за 6 месяцев"
        ],
        [
          "+42%",
          "набор в классы и группы"
        ],
        [
          "+82",
          "отзыва в Яндекс Бизнес за полгода"
        ]
      ]
    },
    {
      "id": "tosun",
      "client": "To Sun Travel",
      "field": "Сервис трансферов",
      "geo": "Анталия, Турция",
      "img": "assets/cases/tosun.jpg",
      "tags": [
        "site",
        "context"
      ],
      "services": [
        "Сайт",
        "Google Ads"
      ],
      "summary": "Продающий сайт на 3 языках и Google Ads для сервиса трансферов. Рост заявок и бронирований по Турции.",
      "metrics": [
        [
          "+85%",
          "заявок за первые 2 месяца"
        ],
        [
          "ROAS 450%",
          "за первые 3 месяца"
        ],
        [
          "65%",
          "трафика из поиска Google Ads"
        ],
        [
          "−18%",
          "затрат на рекламу после оптимизации"
        ]
      ]
    },
    {
      "id": "massiv",
      "client": "Русское Ремесло",
      "field": "Производство мебели из массива",
      "geo": "Москва",
      "img": "assets/cases/massiv-mebel.jpg",
      "tags": [
        "context",
        "proizvod",
        "analytics"
      ],
      "services": [
        "Яндекс Директ",
        "Сквозная аналитика"
      ],
      "summary": "Продвижение производства премиальной мебели из массива дерева. Точечная работа с платёжеспособной аудиторией и рост доли целевых заявок.",
      "metrics": [
        [
          "+57%",
          "целевых заявок за 3 месяца"
        ],
        [
          "ROI 264%",
          "окупаемость рекламы"
        ],
        [
          "−29%",
          "стоимость лида (CPL)"
        ],
        [
          "31% → 58%",
          "доля целевых заявок"
        ]
      ]
    },
    {
      "id": "profildoors",
      "client": "Профиль Дорс",
      "field": "Сеть салонов межкомнатных дверей",
      "geo": "Москва",
      "img": "assets/cases/profildoors.jpg",
      "tags": [
        "context",
        "geo",
        "target",
        "analytics"
      ],
      "services": [
        "Яндекс Директ",
        "VK Ads",
        "Яндекс Бизнес",
        "Сквозная аналитика"
      ],
      "summary": "Комплексная Digital-стратегия для сети салонов межкомнатных дверей.",
      "metrics": [
        [
          "+45%",
          "продаж за 2 месяца"
        ],
        [
          "+4,6%",
          "конверсия сайта"
        ],
        [
          "−38%",
          "стоимость лида (CPL)"
        ],
        [
          "ДРР 7,3%",
          "доля рекламных расходов"
        ]
      ]
    },
    {
      "id": "ipapa",
      "client": "iPapa",
      "field": "Магазин техники Apple",
      "geo": "Тверь",
      "img": "assets/cases/ipapa.jpg",
      "tags": [
        "context",
        "geo",
        "analytics"
      ],
      "services": [
        "Яндекс Директ",
        "Яндекс Бизнес",
        "Сквозная аналитика"
      ],
      "summary": "Комплексная стратегия продвижения для официального реселлера Apple в Твери.",
      "metrics": [
        [
          "+53%",
          "продаж"
        ],
        [
          "ROI 420%",
          "рекламы"
        ],
        [
          "33%",
          "лидов даёт Яндекс Бизнес"
        ],
        [
          "−45%",
          "стоимость заявки (CPL)"
        ]
      ]
    },
    {
      "id": "royalneva",
      "client": "Royal Neva",
      "field": "Караоке-клуб",
      "geo": "Санкт-Петербург",
      "img": "assets/cases/royalneva.jpg",
      "tags": [
        "site",
        "context",
        "geo",
        "horeca",
        "analytics"
      ],
      "services": [
        "Разработка сайта",
        "Яндекс Директ",
        "Яндекс Бизнес",
        "SEO",
        "Сквозная аналитика"
      ],
      "summary": "Комплексный запуск и продвижение нового караоке-клуба: сайт с онлайн-бронированием, реклама и сквозная аналитика.",
      "metrics": [
        [
          "+78%",
          "заявок за первые 2 месяца"
        ],
        [
          "ROI 351%",
          "рекламы"
        ],
        [
          "−31%",
          "стоимость лида (CPL)"
        ],
        [
          "12 000+",
          "посетителей сайта в месяц"
        ]
      ]
    },
    {
      "id": "bmclinic",
      "client": "BM Clinic",
      "field": "Многопрофильная клиника",
      "geo": "Москва",
      "img": "assets/cases/bmclinic.jpg",
      "imgPos": "center top",
      "tags": [
        "context",
        "geo",
        "med",
        "analytics"
      ],
      "services": [
        "Яндекс Директ",
        "Яндекс Бизнес",
        "Сквозная аналитика"
      ],
      "summary": "Комплексная стратегия digital-продвижения для многопрофильного медицинского центра: рост онлайн-записей и контроль рекламных расходов.",
      "metrics": [
        [
          "+120%",
          "записей на приём"
        ],
        [
          "5 900 ₽",
          "средний чек (+25%)"
        ],
        [
          "55%",
          "онлайн-записей через Яндекс Бизнес"
        ],
        [
          "−28%",
          "неэффективных расходов на рекламу"
        ]
      ]
    }
  ],
  "certsHead": {
    "eyebrow": "Подтверждённая экспертиза",
    "heading": "Сертификаты Яндекса и Google",
    "lead": "Официальные сертификации Яндекса и партнёрский статус Roistat. Нажмите на сертификат, чтобы открыть его в полном размере."
  },
  "certs": [
    {
      "title": "Официальный партнёр Roistat",
      "issuer": "Roistat · сквозная аналитика",
      "img": "assets/certs/roistat.jpg",
      "file": "assets/certs/roistat.pdf"
    },
    {
      "title": "Яндекс Директ — Продвинутый",
      "issuer": "Яндекс · сертификат специалиста",
      "img": "assets/certs/direct-pro.jpg",
      "file": "assets/certs/direct-pro.pdf"
    },
    {
      "title": "Яндекс Директ — Базовый",
      "issuer": "Яндекс · сертификат специалиста",
      "img": "assets/certs/direct-base.jpg",
      "file": "assets/certs/direct-base.pdf"
    },
    {
      "title": "Яндекс Метрика",
      "issuer": "Яндекс · сертификат специалиста",
      "img": "assets/certs/metrika.jpg",
      "file": "assets/certs/metrika.pdf"
    },
    {
      "title": "Медийная реклама",
      "issuer": "Яндекс · сертификат специалиста",
      "img": "assets/certs/media.jpg",
      "file": "assets/certs/media.pdf"
    },
    {
      "title": "Реклама мобильных приложений",
      "issuer": "Яндекс · сертификация специалиста",
      "img": "assets/certs/mobile.jpg",
      "file": "assets/certs/mobile.pdf"
    },
    {
      "title": "Геоперформанс и Яндекс Бизнес",
      "issuer": "Яндекс · сертификация специалиста",
      "img": "assets/certs/yandex-geo.jpg",
      "file": "assets/certs/yandex-geo.pdf"
    },
    {
      "title": "Google Реклама — Поиск",
      "issuer": "Google Ads · поисковые кампании",
      "img": "assets/certs/google-search.jpg",
      "file": "assets/certs/google-search.pdf"
    },
    {
      "title": "Google Реклама — КМС",
      "issuer": "Google Ads · контекстно-медийная сеть",
      "img": "assets/certs/google-display.jpg",
      "file": "assets/certs/google-display.pdf"
    }
  ],
  "contacts": {
    "eyebrow": "Контакты",
    "heading": "Заказать интернет-маркетинг под ключ",
    "lead": "Напишите в Telegram или оставьте номер — я перезвоню лично, без колл-центра и менеджеров.",
    "phone": "+7 (996) 347-00-65",
    "phoneTel": "+79963470065",
    "telegram": "@Daniil_065",
    "telegramUrl": "https://t.me/Daniil_065",
    "maxUrl": "https://max.ru/u/f9LHodD0cOKhyIzKq01tP4W7NPCgguZmr-6XQ2vXMOaCb3gg1L1a1m4PP0c",
    "formTitle": "Заказать звонок"
  },
  "quiz": [
    {
      "q": "Какая задача сейчас важнее?",
      "opts": [
        "Привлечь заявки",
        "Сделать сайт",
        "Настроить аналитику",
        "Взять маркетинг целиком"
      ]
    },
    {
      "q": "В какой нише ваш бизнес?",
      "opts": [
        "Услуги",
        "Товары / e-com",
        "Производство",
        "Другое"
      ]
    },
    {
      "q": "Что с рекламой сейчас?",
      "opts": [
        "Не запускали",
        "Запускали сами",
        "Был подрядчик",
        "Работает, но слабо"
      ]
    },
    {
      "q": "Бюджет на маркетинг в месяц",
      "opts": [
        "До 50 000 ₽",
        "50–150 000 ₽",
        "150–300 000 ₽",
        "Более 300 000 ₽"
      ]
    }
  ]
}
;
